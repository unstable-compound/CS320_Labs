...
public class MiniParser extends Phase {
    ...
    public Block parseProgram() throws Failure {
        if (program==null) {
            ...
            program = new Block(parseStmts(0, MiniToken.ENDINPUT));
        }
        return program;
    }
     ...
     */
    private Stmt[] parseStmts(int n, MiniToken term) throws Failure {
    ...
    private Stmt parseStmt() throws Failure {
        switch (lexer.getToken()) {
            case SEMI   : return parseEmpty();
            case OBRACE : return parseBlock();
    ...
    private Stmt parseBlock() throws Failure {
        lexer.nextToken(/* { */);
        Stmt[] stmts = parseStmts(0, MiniToken.CBRACE);
        lexer.nextToken(/* } */);
    ...
    private Stmt parseIf() throws Failure {
        ...
        Stmt ifFalse = lexer.match(MiniToken.ELSE) ? parseStmt() : new Empty(lexer.getPos());
        return new If(pos, test, ifTrue, ifFalse);
    }

    private Expr parseTest() throws Failure {
        require(MiniToken.OPAREN);
        Expr test = parseExpr();
        require(MiniToken.CPAREN);
        return test;
    }
    ...
    private Id[] parseIds(int n) throws Failure {
        if (lexer.getToken()==MiniToken.ID) {
            Id   id  = new Id(lexer.getPos(), lexer.getLexeme());
            lexer.nextToken(/* ID */);
            Id[] ids = lexer.match(MiniToken.COMMA) ? parseIds(n+1) : new Id[n+1];
            ids[n]   = id;
            return ids;
        }
    ...
    private Expr parseMult() throws Failure {
        Expr e = parseUnary();
        for (;;) {
            if (lexer.getToken()==MiniToken.MUL) {
                Position pos = lexer.getPos();
                lexer.nextToken(/* * */);
    ...
    private Expr parseAdd() throws Failure {
        Expr e = parseMult();
        for (;;) {
            if (lexer.getToken()==MiniToken.PLUS) {
                Position pos = lexer.getPos();
                lexer.nextToken(/* + */);
                e = new Add(pos, e, parseMult());
            } else if (lexer.getToken()==MiniToken.MINUS) {
                Position pos = lexer.getPos();
                lexer.nextToken(/* - */);
    ...
    private Expr parseRel() throws Failure {
        Expr e = parseAdd();
        for (;;) {
            if (lexer.getToken()==MiniToken.LT) {
                Position pos = lexer.getPos();
                lexer.nextToken(/* < */);
                e = new Lt(pos, e, parseAdd());
            } else if (lexer.getToken()==MiniToken.LTE) {
                Position pos = lexer.getPos();
                lexer.nextToken(/* <= */);
                e = new Lte(pos, e, parseAdd());
            } else if (lexer.getToken()==MiniToken.GT) {
                Position pos = lexer.getPos();
                lexer.nextToken(/* > */);
                e = new Gt(pos, e, parseAdd());
            } else if (lexer.getToken()==MiniToken.GTE) {
                Position pos = lexer.getPos();
                lexer.nextToken(/* >= */);
    ...
    private Expr parseEql() throws Failure {
        Expr e = parseRel();
        for (;;) {
            if (lexer.getToken()==MiniToken.EQEQ) {
                Position pos = lexer.getPos();
                lexer.nextToken(/* == */);
                e = new Eql(pos, e, parseRel());
            } else if (lexer.getToken()==MiniToken.NEQ) {
                Position pos = lexer.getPos();
                lexer.nextToken(/* != */);
    ...
    private Expr parseBAnd() throws Failure {
        Expr e = parseEql();
        while (lexer.getToken()==MiniToken.BAND) {
            Position pos = lexer.getPos();
            lexer.nextToken(/* & */);
    ...
    private Expr parseBXor() throws Failure {
        Expr e = parseBAnd();
        while (lexer.getToken()==MiniToken.BXOR) {
            Position pos = lexer.getPos();
            lexer.nextToken(/* ^ */);
    ...
    private Expr parseBOr() throws Failure {
        Expr e = parseBXor();
        while (lexer.getToken()==MiniToken.BOR) {
            Position pos = lexer.getPos();
            lexer.nextToken(/* | */);
    ...
    private Expr parseLAnd() throws Failure {
        Expr e = parseBOr();
        while (lexer.getToken()==MiniToken.LAND) {
            Position pos = lexer.getPos();
            lexer.nextToken(/* && */);
    ...
    private Expr parseLOr() throws Failure {
        Expr e = parseLAnd();
        while (lexer.getToken()==MiniToken.LOR) {
            Position pos = lexer.getPos();
            lexer.nextToken(/* || */);
    ...
    private Expr parseExpr() throws Failure {
        Expr e = parseLOr();
        if (lexer.getToken()==MiniToken.EQ) {
            Position pos = lexer.getPos();
            lexer.nextToken(/* = */);
    ...
    private void require(MiniToken tok) throws Failure {
        if (lexer.getToken()!=tok) {
            throw new Failure(lexer.getPos(), "Missing " + tok.tokenName);
        }
        lexer.nextToken(/* tok */);
    }
