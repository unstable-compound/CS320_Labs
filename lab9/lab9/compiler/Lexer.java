...
/** A framework for building lexical analyzers, parameterized over a type Token of token codes.
 *  Token codes are tested for equality using ==, so Token should normally be an enumeration type. 
 *
 *  Conventions: when a token has been recognized, save its token
 *  code in the variable `token' and any corresponding text,
 *  if required, in the variable `lexemeText'.
 */
public abstract class Lexer<Token> extends Phase {
     ...
     *  token.
     */
    public Lexer(Handler handler, Token firstToken) {
        super(handler);
        token = firstToken;
    }

    /** Read the next token and return the corresponding code.
